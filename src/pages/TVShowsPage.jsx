...
import {
  ...
  getTopRatedTV,
  getTrendingTV,
} from "../services/tvshows.api";

const categories = [
  { key: "popular", label: "Popular", fetcher: getPopularTV },
  { key: "top-rated", label: "Top Rated", fetcher: getTopRatedTV },
  { key: "airing-today", label: "Airing Today", fetcher: getAiringTodayTV },
  { key: "on-the-air", label: "On The Air", fetcher: getOnTheAirTV },
  { key: "trending", label: "Trending", fetcher: getTrendingTV },
];
...
const TVShowsPage = () => {
  ...
  const fetchShows = async (nextPage = 1, reset = false) => {
    ...
    try {
      ...
      if (reset) {
        ...
        setShows((prevShows) => [...prevShows, ...response]);
      }
    ...
    <section className="min-h-[calc(100vh-76px)] bg-[radial-gradient(circle_at_top_right,_rgba(229,9,20,0.18),_transparent_30rem)] py-8 sm:py-10">
      ...
      <div className="mb-6 flex items-center justify-between gap-3 text-sm text-gray-300">
        <div>
          ...
          <span className="ml-2 text-gray-400">TV shows</span>
        </div>
